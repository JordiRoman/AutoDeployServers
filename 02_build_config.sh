...
# Creamos la ssh key
if [ ! -e ${WORK_DIR}/key-${HOSTNAME}.pub ]; then
	ssh-keygen -t rsa -b 4096 -C "${HOST_NAME} ssh key" -f ${WORK_DIR}/key-${HOSTNAME} -N ${PASSWORD}
fi
SSH_PUB_KEY=$(cat ${WORK_DIR}/key-${HOSTNAME}.pub )
